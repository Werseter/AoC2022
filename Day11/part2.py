...
div = prod(int(entry[2].split()[-1]) for entry in data)
monkeys = [[[int(x) for x in rules[0].split(',')], partial(lambda dst, md, x: monkeys[dst[x % md == 0]][0].append(x),
            ...
            partial(lambda evl, old: eval(evl) % div, rules[1].split('=')[1])] for rules in data]
[[[t(o(i)) for i in l] and c.update({0: c[0] + len(l)}) or l.clear() for l, t, c, o in monkeys] for _ in range(10000)]
